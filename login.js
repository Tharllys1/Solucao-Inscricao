...
function salvarUser() {
    ...
    if (NumeroCamisa && Participantes && Sexo) {
...
// FUNÇÃO CRIA LISTA
function crialista() {
    let tabela = "<tr><th>Participantes</th><th>N º da camisa</th><th>Genêro</th><th>Ações</th></tr>";
    for (let i = 0; i < Camisas.length; i++) {
        tabela += "<tr><td>" + Nome[i] + "</td><td>" + Camisas[i]  + "</td><td>" + Genero[i] + "</td><td><button class='btn btn-success' onclick='editar(" + i + ")'>Editar</button><button class='btn btn-danger' onclick='excluir(" + i + ")'>Excluir</button></td></tr>";
    }
